...
internal sealed class YoloHbbDetectionService : IYoloHbbDetectionService
{
    public HbbDetectionResult[] Detect(
        string modelPath,
        string imagePath,
    ...
    )
    {
        ...
        using var session = new InferenceSession(modelPath);
        return [.. session.Detect(image, labelArray, detectionOptions)];
    }
    ...
    )
    {
        ...
        using var session = new InferenceSession(modelPath);
        return session.DetectAndDraw(image, labelArray, detectionOptions, drawOptions);
    }
